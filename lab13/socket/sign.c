...
void sign_up() {
		...
		while(1) {
			...
			send(c_sock, new_id, strlen(new_id), 0);
			recv(c_sock, exist, sizeof(char)*11, 0);
			if(!strcmp(exist, "exist")) {
					...
					refresh();
					endwin();
					send(c_sock, new_pwd, sizeof(char)*20, 0);
					send(c_sock, new_num, sizeof(char)*20, 0);
					send(c_sock, new_name, sizeof(char)*20, 0);
					send(c_sock, new_major, sizeof(char)*20, 0);
					break;
			}
		}
...
void sign_in() {
		...
		while(1) { //if tried 3 times, break
			...
			send(c_sock, user_id, sizeof(char)*20, 0);
			send(c_sock, user_pwd, sizeof(char)*20, 0);
			recv(c_sock, exist, sizeof(char)*12, 0);

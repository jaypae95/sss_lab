...
int main() {
		...
		pthread_mutex_init(&sync_mutex, NULL); //상호 배제 락 초기화
...
void * philosopher (void * args) {
		...
		for(k=1 ; ;k++ ) {
				...
				if(chopstick[left] == 1 && chopstick[right] == 1) { //왼쪽 젓가락과 오른쪽 젓가락이 둘다 식탁에 놓여 있으면
						chopstick[right] = 0; //두 젓가락을 모두 든다
						chopstick[left] = 0; // "
						p_left = 1; // "
						p_right = 1; // "
				}
				...
				else {		//양손에 젓가락이 없거나 하나 있는 경우 1초 동안 생각한다.

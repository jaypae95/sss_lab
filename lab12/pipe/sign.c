...
void sign_up() {
		...
		while(1) {
			...
			write(fd1[1], new_id, strlen(new_id));
			read(fd2[0], exist, sizeof(char)*11);
			if(!strcmp(exist, "exist")) {
					...
					refresh();
					endwin();
					write(fd1[1], new_pwd, sizeof(char)*20);
					write(fd1[1], new_num, sizeof(char)*20);
					write(fd1[1], new_name, sizeof(char)*20);
					write(fd1[1], new_major, sizeof(char)*20);
					break;
			}
		}
...
void sign_in() {
		...
		while(1) { //if tried 3 times, break
			...
			write(fd1[1], user_id, sizeof(char)*20);
			write(fd1[1], user_pwd, sizeof(char)*20);
			read(fd2[0], exist, sizeof(char)*12);
